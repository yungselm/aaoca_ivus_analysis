...
class LoadIndividualData:
    ...
    def find_dirs(self) -> Tuple[Path, Path]:
        ...
        loess_dir = self.working_dir / "ivus"
        ...
        loess_dir = find_matching_dir(loess_dir)
        ...
        if loess_dir is None:
            raise FileNotFoundError(f"No Loess data directory for {self.id} in {loess_dir}")
    ...
    def _read_loess_phase(filename: Path, phase: str = "D") -> pd.DataFrame:
        ...
        measurement_blocks = {
            ...
            'elliptic_ratio': ['elliptic_ratio', 'elliptic_ratio_ci_lower', 'elliptic_ratio_ci_upper'],
            'shortest_distance': ['shortest_distance', 'shortest_ci_lower', 'shortest_ci_upper']
        }
        ...
        grouped_df = (
            combined_df
            .groupby(['position', 'measurement', 'phase'], as_index=False)
            .agg({
                'value': 'mean',
                'ci_lower': 'mean',
                'ci_upper': 'mean'
    ...
    def _read_loess_global(filename: Path) -> pd.DataFrame:
        ...
        return combined_df.set_index('position').sort_index().pipe(lambda df: 
            df[['measurement', 'phase', 'value', 'ci_lower', 'ci_upper']])