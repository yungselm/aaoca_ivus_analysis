...
class GlobalStats:
    ...
    def global_stats(self):
        ...
        self.univariate_logistic_regression(
            target_var_cont="pdpa_mean_stress", target_cutoff=0.8
        )
        self.univariate_logistic_regression(
            target_var_cont="iFR_mean_stress", target_cutoff=0.8
        )
    ...
    def pairwise_rest_stress(self):
        ...
        self.global_data.columns = self.global_data.columns.str.replace(
            "dobu", "stress", regex=False
        )
        # 1. Filter numeric columns only
        numeric_cols = self.global_data.select_dtypes(
            include=np.number
        ).columns.tolist()

        valid_cols = [
            col
            for col in self.global_data.columns
        ...
        merged_df[["metric", "phase"]] = pd.DataFrame(
            split.tolist(), index=merged_df.index
        )
        merged_df = merged_df.dropna(
            subset=["phase"]
        )  # Remove rows where phase wasn't found
        ...
        pivoted = (
            merged_df.pivot_table(
                index=["patient_id", "metric"], columns="phase", values="value"
            )
            .reset_index()
            .dropna(subset=["rest", "stress"])
        )  # Forcefully drop rows with NaN in either phase
    ...
    def pairwise_pressure_cutoff(self, cutoff_var="iFR_mean_stress", cutoff=0.8):
        ...
        logger.info(
            f"Group sizes - Above: {len(group_above)}, Below/Equal: {len(group_below)}"
        )
        ...
        for metric in metrics:
            ...
            if len(data_above) < 6 or len(data_below) < 6:
                logger.debug(
                    f"Skipping {metric}: insufficient samples (Above={len(data_above)}, Below={len(data_below)})"
                )
                continue
    ...
    def plot_global_change(self, mode="pressure", phases=None):
        ...
        # 4) Check columns
        missing = [
            c for c in left_cols + right_cols if c not in self.global_data.columns
        ]
        if missing:
        ...
        for ax, cols, title, idx in zip(
            axes, [left_cols, right_cols], [left_title, right_title], [0, 1]
        ):
    ...
    def univariate_logistic_regression(
        ...
        output_file = os.path.join(
            self.output_dir, f"logreg_univariate_{target_var_cont}_{target_cutoff}.csv"
        )
